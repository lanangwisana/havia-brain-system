<div class="card">
    ...
    <?php if ($can_edit_estimates) { ?>
        <div class="card-body">
            <?php echo modal_anchor(get_uri("tasks/modal_form"), "<i data-feather='plus' class='icon-16'></i> " . app_lang('add_task'), array("class" => "", "data-post-estimate_id" => $estimate_id, "title" => app_lang('add_task'))); ?>
        </div>
    <?php } ?>

    <div class="table-responsive">
        <table id="estimate-details-page-task-table" class="display no-thead b-t b-b-only no-hover hide-dtr-control hide-status-checkbox" cellspacing="0" width="100%">
        </table>
    </div>
</div>
...
<script type="text/javascript">
    $(document).ready(function () {
        $("#estimate-details-page-task-table").appTable({
            source: '<?php echo_uri("tasks/list_data/estimate/" . $estimate_id) ?>' + '/1',
            order: [[0, "desc"]],
            hideTools: true,
            displayLength: 100,
            stateSave: false,
            ...
            reloadHooks: [{
                    type: "app_form",
                    id: "task-form",
                    filter: {estimate_id: "<?php echo $estimate_id ?>"},
                },
                {
                    type: "app_table_row_update",
                    tableId: "estimate-details-page-task-table"
                },
                {
                    type: "app_modifier",
                    group: "task_info"
