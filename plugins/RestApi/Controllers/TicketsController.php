...
class TicketsController extends Rest_api_Controller 
{
	 ...
	 *
	 * @apiParam (path) {Number} ticketid Mandatory ticket unique ID
	 *
	 ...
	 * @apiHeader {String} authtoken Authentication token, generated from admin area
	 *
	 * @apiParam (body) {string} title Mandatory Ticket title
	 * @apiParam (body) {int} client_id Mandatory Ticket client_id
	 * @apiParam (body) {int} requested_by_id Mandatory Ticket requested_by_id
	 * @apiParam (body) {int} ticket_type_id Mandatory Ticket ticket_type_id
	 * @apiParam (body) {string} description Mandatory Ticket description
	 * @apiParam (body) {int} assigned_to Mandatory Ticket assigned_to
	 * @apiParam (body) {string} ticket_labels Optional Ticket ticket_labels 
	 *
	 ...
	 * @apiHeader {String} authtoken Authentication token, generated from admin area
	 *
	 * @apiParam (path) {Number} id Ticket unique ID
	 *
	 * @apiParam (body) {string} title Mandatory Ticket title
	 * @apiParam (body) {int} client_id Mandatory Ticket client_id
	 * @apiParam (body) {int} requested_by_id Mandatory Ticket requested_by_id
	...
	{ 		
		...
		if (!is_numeric($id) || empty($is_ticket_exists)) {
			return $this->fail(app_lang('ticket_id_invalid'), 404);
		}
        ...
        return $this->fail(app_lang('ticket_update_fail'), 400);
	}
	...
	public function delete($id = null) 
	{
		...
		if ($this->tickets_model->get_details(['id' => $id])->getResult()) {
			...
			return $this->fail(app_lang('ticket_delete_fail'), 400);
		}
		return $this->fail(app_lang('ticket_delete_fail'), 400);
	}
}
