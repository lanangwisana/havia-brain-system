...
class InvoicesController extends Rest_api_Controller 
{
	...
	/**
	 * @api {get} /api/invoices/:invoiceid List all Invoice information
	 * @apiVersion 1.0.0
	 * @apiName getInvoices
	 * @apiGroup Invoices
	 * @apiHeader {String} authtoken Authentication token, generated from admin area
	 *
	 * @apiParam (path) {Number} invoiceid Mandatory invoice unique ID
	 *
	...
	/**
	 * @api {post} api/invoices Add an Invoice
	 * @apiVersion 1.0.0
	 * @apiName create
	 * @apiGroup Invoices
	 *
	 * @apiHeader {String} authtoken Authentication token, generated from admin area
	 *
	 * @apiParam (body) {string} invoice_bill_date Optional invoice bill date
	 * @apiParam (body) {string} invoice_due_date Mandatory invoice due date
	 * @apiParam (body) {string} invoice_client_id Mandatory invoice client id
	 * @apiParam (body) {string} invoice_project_id Optional invoice project id
	 * @apiParam (body) {string} tax_id Optional invoice tax id
	 * @apiParam (body) {string} tax_id2 Optional invoice tax id2
	 * @apiParam (body) {string} tax_id3 Optional invoice tax id3
	 * @apiParam (body) {string} recurring Optional invoice recurring
	 * @apiParam (body) {string} invoice_note Optional invoice note
	 * @apiParam (body) {string} labels Optional invoice labels
	 *
	 ...
	 * 	  	  'created_by' => number Optional RISE user ID. Default: 1. Use for proper attribution.
	 *
	...
	public function create() 
	{
		...
		return $this->fail(app_lang('invoice_add_fail'), 400);
	}
	...
	/**
	 * @api {put} api/invoices/:id Edit an Invoice
	 * @apiVersion 1.0.0
	 * @apiName update
	 * @apiGroup Invoices
	 *
	 * @apiParam (path) {Number} id Invoice unique ID
	 * @apiParam (body) {string} invoice_bill_date Optional Invoice Bill Date
	 * @apiParam (body) {string} invoice_due_date Mandatory Invoice Due Date
	 * @apiParam (body) {string} invoice_client_id Mandatory Invoice Client id
	 * @apiParam (body) {string} invoice_project_id Optional Invoice Project id
	 ...
	 * @apiParam (body) {string} recurring Optional Invoice Recurring
	 * @apiParam (body) {string} invoice_note Optional Invoice note
	 * @apiParam (body) {string} labels Optional Invoice Label
	 *
	 *
	...
	public function update($id = null) 
	{
		...
		if (!is_numeric($id) || empty($is_invoice_exits)) {
			return $this->fail(app_lang('invoice_id_invalid'), 404);
		}
		...
		return $this->fail(app_lang('invoice_update_fail'), 400);
	}

	/**
	 * @api {delete} api/invoices/:id Delete an Invoice
	 * @apiVersion 1.0.0
	 * @apiName Delete
	 * @apiGroup Invoices
	 *
	...
	public function delete($id = null) 
	{
		...
		if (!empty($invoice_info)) {
			...
			return $this->fail(app_lang('invoice_delete_fail'), 400);
		}
		return $this->fail(app_lang('invoice_delete_fail'), 400);
	}
}
